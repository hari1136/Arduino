...
void ADE7758::Init(void) {
  pinMode((AFECS + 3), OUTPUT);
  digitalWrite((AFECS + 3), HIGH); //disabled by default
  // SPI Init
...
*/
int32_t ADE7758::read16(char reg) {
  enableChip();
  int32_t b1, b0;
  delayMicroseconds(10);
  SPI.transfer(reg);
  ...
  delayMicroseconds(10);
  disableChip();
  return (int32_t)b1 << 8 | (int32_t)b0;
}
...
*/
int32_t ADE7758::read24(char reg) {
  enableChip();
  int32_t b2, b1, b0;
  delayMicroseconds(10);
  SPI.transfer(reg);
...
int ADE7758::getVpeak() {
  return read8(VPEAK);
}

int ADE7758::getIpeak() {
  return read8(IPEAK);
}
void ADE7758::setVPINTLVL(int m) {
  write8(VPINTLVL, m);
}

void ADE7758::setIPINTLVL(int m) {
  write8(IPINTLVL, m);
}
void ADE7758::setSAGLVL(int m) {
  write8(SAGLVL, m);
}

int ADE7758::getVPINTLVL() {
  return read8(VPINTLVL);
}
int ADE7758::getSAGLVL() {
  return read8(SAGLVL);
}
...
char ADE7758::setPotLine(char Phase, int Ciclos)
{
  ...
  //if(Intf==0)
  //if (!(intrstat & 0x001C0))
  while (!(getStatus() & LENERGY))  // wait to finish accumulating
  { // wait for the selected interrupt to occur
